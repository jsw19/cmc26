...
import { estimateTradeInValue } from '../sdk/tradeInEstimate';
import type { LocationInfo, Severity, TradeInEstimate, VehicleInfo } from '../sdk/types';

export type TradeInStatus = 'idle' | 'loading' | 'done' | 'permission_denied' | 'error';

export function useTradeInEstimate() {
  const [status, setStatus] = useState<TradeInStatus>('idle');
  ...
  async function getEstimate(
    ...
    try {
      ...
      const estimate = estimateTradeInValue(vehicleInfo, overallSeverity, locationInfo);
      setStatus('done');
      return estimate;
    } catch (err) {
